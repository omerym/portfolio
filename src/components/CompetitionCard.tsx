...
const CompetitionCard: React.FC<CompetitionCardProps> = ({
    ...
    <div className="bg-white dark:bg-slate-800 rounded-lg shadow p-6 border border-gray-200 dark:border-slate-700 flex flex-col gap-4">
        ...
        <div className="flex gap-3 mt-2">
            <a href={solutionUrl} target="_blank" rel="noopener noreferrer" className="px-4 py-2 rounded bg-blue-600 text-white font-semibold hover:bg-blue-700 transition-colors flex items-center">
                {showIcon && getSolutionIcon(solutionUrl)}
                Solution
            </a>
            <a href={competitionUrl} target="_blank" rel="noopener noreferrer" className="px-4 py-2 rounded bg-amber-500 text-white font-semibold hover:bg-amber-600 transition-colors">Competition</a>
        </div>
    </div>
);
